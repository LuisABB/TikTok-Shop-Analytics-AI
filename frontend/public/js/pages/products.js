...
const ProductsPage = (() => {
  ...
  function renderTopProductsChart(data) {
    ...
    const values = data.map(p => p[currentMetric] || 0);
    const isMonetary = currentMetric === 'gmv';

    mountChart('chart-top-products', {
      ...defaultChartOptions(),
      chart:   { type: 'bar', height: 300 },
      series:  [{ name: currentMetric.toUpperCase(), data: values }],
  ...
  function renderProductsTable(data) {
    ...
    if (!data.length) {
      tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4 small">Sin datos de productos. Importa un CSV de "Product Card Traffic Stats".</td></tr>';
      return;
    }
    tbody.innerHTML = data.map(p => `
      <tr>
